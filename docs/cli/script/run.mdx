...
❖ Arcadia CLI - 执行代码文件
  ...
  命令选项：
    ...
    -r, --recombine-env         变量重组 - 按照指定顺序重新组合复合变量的成员值，选项后需跟变量名称、分隔符、重组表达式
...
| `-a`, `--agent`                | 网络代理 | 否 | 为 JavaScript 和 TypeScript 代码文件启用全局 HTTP/HTTPS 代理，使用该功能需配置 `GLOBAL_AGENT_HTTP_PROXY` 环境变量，更多配置方法详见 [global-agent](https://www.npmjs.com/package/global-agent) |
...
<Accordions type="single">
  ...
  <Accordion title="关于执行参数和传递选项的区别">
  ...
  <Accordion title="关于 --agent 网络全局代理命令选项">
    根据其官方文档说明，需要配置 `GLOBAL_AGENT_HTTP_PROXY` 或 `GLOBAL_AGENT_HTTPS_PROXY` 环境变量，具体详见 [global-agent](https://www.npmjs.com/package/global-agent)
  </Accordion>

  <Accordion title="什么是守护进程？">

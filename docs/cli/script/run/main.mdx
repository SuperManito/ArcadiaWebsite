...
| 选项 | 用途 | 选项值 | 描述 |
| :-: | :-: | :-: | ----- |
| `-l`, `--loop`                 | 循环运行 | ✓ | 连续多次运行代码文件，选项后需跟 _循环次数(正整数)_，该选项与 **等待执行** 和 **延迟执行** 参数同时使用时仍然有效互不干涉 |
| `-s`, `--silent`               | 静默运行 | | 静默运行任务不推送任何通知消息 |
| `-w`, `--wait`                 | 推迟执行 | ✓ | 等待指定时间后再运行代码文件，选项后需跟 _等待时间单位_ 作为参数值，具体参照 [sleep](https://www.runoob.com/linux/linux-comm-sleep.html) 命令的用法 |

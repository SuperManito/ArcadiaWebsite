...
<Cards>
  <Card
    icon={<Download />}
    title="OpenAPI 规范文件"
    href="https://github.com/SuperManito/ArcadiaWebsite/blob/main/src/openapi.yaml"
  >
...
后端使用 [<Icon>logos-nodejs-icon</Icon> Node.js](https://nodejs.org) 搭建，所以如果你想使用这个功能则需要会写一些 JavaScript 代码
...
导出函数以及函数传参是固定的，请参考下方的简单示例

<details>
<summary>点此展开查看代码示例</summary>

```js title="extra_server.js" lineNumbers
export default async function main(api, resTemplate, logger) {
    // api express框架实例
...
`/api/open/extra` 该系列接口仍需要通过鉴权才可以访问，但是目前没有实行权限细粒化管理，因此任何有效令牌都可以访问

...
<Callout type="info">请不要频繁调用修改类接口，因为每次更新数据后都会重新生成批量声明脚本</Callout>
...
<Tabs items={['普通变量 / 复合变量的值', '复合变量(组)']}>

<Tab value="普通变量 / 复合变量的值">
...
<Callout type="info">总结：除复合变量値以外对于其它变量记录的排序无实际意义且操作方向相反，因为分页数据是倒序返回的，即最近添加的变量靠前展示。</Callout>
